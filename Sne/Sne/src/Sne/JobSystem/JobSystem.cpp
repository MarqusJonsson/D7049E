...
void Initialize()
{
	...
	numThreads = eastl::max(1u, numCores - 1);
...
void Execute(context& ctx, const eastl::function<void()>& job)
{
...
void Dispatch(context& ctx, uint32_t jobCount, uint32_t groupSize, const eastl::function<void(JobDispatchArgs)>& job)
{
	...
	for (uint32_t groupIndex = 0; groupIndex < groupCount; ++groupIndex)
	{
		...
		const auto& jobGroup = [jobCount, groupSize, job, groupIndex]() {
			...
			const uint32_t groupJobEnd = eastl::min(groupJobOffset + groupSize, jobCount);
